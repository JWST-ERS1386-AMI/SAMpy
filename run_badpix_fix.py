...
def fix_bad_pixels(indir,
    ...
    median_size = 3  # pix
    median_tres = 28. # JK: changed from 28 to 20 in order to capture all bad pixels
    nrefrow = 5
    ...
    for i in range(Nfitsfiles):
        ...
        #    ww_max += [np.unravel_index(np.argmax(median_filter(data[j], size=7)), data[j].shape)] # JK: added median filter to catch PSF center despite hot pixels
        #imtmp = np.median(data,axis=0)
        #maxmed = [np.unravel_index(np.argmax(imtmp),imtmp.shape)]
        for j in range(imsz[0]):
            ww_max+=[[40,45]]
        ...
        for j in range(imsz[0]):
            ...
            for k in range(10):
                ...
                if pxdq_new!=0:
                    print('         Integration: %.0f ; iteration %.0f: %.0f new bad pixels, sdev of norm noise = %.3f' % (j, k + 1, pxdq_new, np.std(fmas_data[pmas])))
        ...
        hdul['DQ'].data = np.squeeze(pxdq) # new dq array
...
def correct_fitsfiles(indir, odir, fitsfiles, pxdq0s, pattern='*calints.fits', show=None, save=False):
    toc = time.time()
